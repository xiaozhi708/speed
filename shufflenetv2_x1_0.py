...
parser.add_argument('--model_type',default='shufflenetv2_x1_0',help='the choice of model option like:mobilenetv2,shufflenetv2_x1_0，vgg16_bn, resnet50,googlenet,densenet121')
...
if __name__=='__main__':
    ...
    # 训练和验证
    best_model, record = train_and_valid(model,train_data_size,train_dataloader,valid_data_size,val_dataloader,loss_func, optimizer)
    # 保存最好的模型和曲线图
    torch.save(best_model.state_dict(), args.model_save_dir+args.model_type+'.pth') #保存最好的模型
    save_acc_loss_image(record) #保存准确率和损失的曲线图
